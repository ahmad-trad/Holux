...
/*=============== VALUE ACCORDION ===============*/
let accordionItem = document.querySelectorAll('.value__accordion-item');

accordionItem.forEach((item) =>{
    let accrodionHeader = item.querySelector('.value__accordion-header');

    accrodionHeader.addEventListener('click', () =>{
        let accordionOpen = document.querySelector('.accordion-open');
        
        tiggleItem(item);

        if(accordionOpen && accordionOpen !== item){
            tiggleItem(accordionOpen);
        }
...
sr.reveal(`.home__search`, {delay: 250});
